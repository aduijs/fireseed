...
function closeMobileMenu(menuEl) {
    if (menuEl.className = menuEl.className.replace(/\btriggered\b/, ""), document.getElementById("menu").className = document.getElementById("menu").className.replace(/\bmobile-show\b/, ""), document.getElementById("menu").className = document.getElementById("menu").className + " mobile-hide", document.getElementById("home"))for (var homeEl = document.getElementById("home"), homeItems = homeEl.getElementsByClassName("item"), i = 0; i < homeItems.length; i++)homeItems[i].className = homeItems[i].className + " menu-closed", homeItems[i].className = homeItems[i].className.replace(/\bmenu-open\b/, "");
...
function openMobileMenu(menuEl) {
    if (menuEl.className = menuEl.className + " triggered", document.getElementById("menu").className = document.getElementById("menu").className.replace(/\bmobile-hide\b/, ""), document.getElementById("menu").className = document.getElementById("menu").className + " mobile-show", document.getElementById("home"))for (var homeEl = document.getElementById("home"), homeItems = homeEl.getElementsByClassName("item"), i = 0; i < homeItems.length; i++)homeItems[i].className = homeItems[i].className + " menu-open", homeItems[i].className = homeItems[i].className.replace(/\bmenu-closed\b/, "");
    if (document.getElementById("two-column")) {
        var twoColumnEl = document.getElementById("two-column"), moduleListEl = twoColumnEl.getElementsByClassName("module_list")[0];
...
traskindustries.config(["$routeProvider", "$locationProvider", function ($routes) {
    $routes.when("/home", {controller: "HomeController", templateUrl: "views/home.html"}), $routes.when("/innovation", {controller: "InnovationController", templateUrl: "views/two-column.html"}), $routes.when("/genetics", {controller: "GeneticsController", templateUrl: "views/two-column.html"}), $routes.when("/media", {controller: "MediaController", templateUrl: "views/media.html"}), $routes.when("/about", {controller: "AboutController", templateUrl: "views/two-column.html"}), $routes.otherwise({redirectTo: "/home"})
}]);
var isIE8 = -1 != navigator.userAgent.indexOf("MSIE 8"), isIphone = navigator.userAgent.match(/iPhone/i), frameDoneList = [], frameReadyList = [], infoInitArray = [], setupListSpace = function () {
    animateIn();
    var mAI = function (e) {
        e.className += " active"
    }, mAO = function (e) {
        e.className = e.className.split(" active").join("")
    }, constructor = {contents: $$(".item", $$(".window", $$(".module_marquee"))), nav: $$(".button", $$(".pagination", $$(".module_marquee"))), animateInCallback: mAI, animateOutCallback: mAO}, Marq = new traskMarquee(constructor);
    ...
    }, constructor = {contents: $$(".item", $$(".module_list")), scrollContext: ".module_list", animateInCallback: lAI, animateOutCallback: lAO}, List = new traskList(constructor);
    ...
    for (var preloadImageSequence = function (path, total, ext, delay) {
        ...
        var t = listImageSeq[i] || listImageSeq, container = $$(".list_imagesequence", t), sB = getAttr(container, "rel"), fr = getAttr(container, "ref"), iO = {fps: 10, sequenceElement: container, srcBase: sB, totalFrames: fr, spriteElement: $$(".icon", t), autoplay: !1, loop: !0};
    ...
    if (1 == $$(".module_list").length) {
        ...
        if (W() <= 641)var liH = 385; else if (W() <= 1023)var liH = 365; else var liH = 433;
    ...
    if (!result)for (var attrs = ele.attributes, length = attrs.length, i = 0; length > i; i++)attrs[i].nodeName === attr && (result = attrs[i].nodeValue);
    return result
};
...
traskindustries.controller("AboutController", function ($scope, $http) {
    ...
    var setUpHomeSpace = function () {
        var mAI = function (e) {
            ...
            e.className = e.className.split(" active").join("")
        }, constructor = {contents: $$(".item", $$(".window", $$(".module_marquee"))), nav: $$(".button", $$(".pagination", $$(".module_marquee"))), autoAdvance: "true", autoAdvanceTime: 6e3, animateInCallback: mAI, animateOutCallback: mAO}, Marq = new traskMarquee(constructor);
        $$(".module_marquee").Marq = Marq;
        for (var mImageLoaded = function (e) {
            centerImage(e.currentTarget)
        }, centerImage = function (e) {
        ...
        var ytSel = ".yt_container", handleResizeHome = function () {
        ...
        for (var findOutHow = function (e) {
            Marq.pause();
            var menuEl = document.getElementById("mobile-menu"), targ = e.srcElement || e.currentTarget || e, nextSection = targ.getAttribute ? targ.getAttribute("rel") : targ.attributes.rel.value;
            dataLayer.push({pageName: "/FOH/" + nextSection, event: "pageView"}), hasClass(menuEl, "triggered") && (document.getElementById("menu").className = document.getElementById("menu").className.replace(/\bmobile-show\b/, ""), document.getElementById("menu").className = document.getElementById("menu").className + " mobile-hide", menuEl.style.margin = "0", menuEl.className = menuEl.className.replace(/\btriggered\b/, "")), document.width > 1024 ? animateSectionOut(function () {
    ...
    function bindHoverstates(selector) {
        if (!$$(selector).length, $$(selector).length > 1)for (var i = 0; i < $$(selector).length; i++)vine.bind($$(selector)[i], "mouseover", hoverState), vine.bind($$(selector)[i], "mouseout", defaultState); else vine.bind($$(selector), "mouseover", hoverState), vine.bind($$(selector), "mouseout", defaultState)
    }
    ...
    function setItemSizes(selector, elWidth) {
        ...
        if ($$(selector).length > 1)for (var i = 0; i < $$(selector).length; i++)$$(selector)[i].style.width = elWidth; else $$(selector).style.width = elWidth
    }
    ...
    var msnry, windowWidth, baseWidth, lgWidth, currentDateTime, numCols, fadeDuration = 400, fadeDelay = 300, ytLightbox = {_inst: this, _vidID: null, _video: null, openLightbox: function (e) {
        var targ = e.srcElement || e.currentTarget || e;
        ...
        if (!$$(selector).length, $$(selector).length > 1)for (var i = 0; i < $$(selector).length; i++)vine.bind($$(selector)[i], "click", ytLightbox.openLightbox); else vine.bind($$(selector), "click", ytLightbox.openLightbox)
    }}
});
var menuInited = !1;
angular.module('motorhotelApp').controller("MenuCtrl", function ($scope, $http, $location) {
    function socialInit() {
